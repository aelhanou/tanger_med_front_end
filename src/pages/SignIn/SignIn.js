...
export const SignIn = () => {
  ...
  const [email, setEmail] = useState("elhanouniazeddine00@gmail.com");
  const [password, setPassword] = useState("azeddine.1998");
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      let { data } = await axios.post("http://localhost:9998/login", {
        email,
        password,
      });
